// fork of venk customtablist plugin
// usable only in blues server
...
namespace MCGalaxy
{
    public class CustomTabList : Plugin
    {
        ...
        public static string path = "./plugins/CustomTabList2";

        public class Config
        {
            [ConfigString("syntax", "Extra", "[color][username] [pingcolor]")]
            public static string Syntax = "[color][username] [pingcolor]";
        ...
        public static void MakeConfig()
        {
            using (StreamWriter w = new StreamWriter(path + "/config.properties"))
            {
                ...
                w.WriteLine("syntax = [color][nick] [pingcolor]");
                w.WriteLine();
            }
        }
        ...
        void HandleTabListEntryAdded(Entity entity, ref string name, ref string group, Player p)
        {
            ...
            if (pl.Session.Ping.AveragePing() > 0 && pl.Session.Ping.AveragePing() < 50) pingColor = "♪";
            if (pl.Session.Ping.AveragePing() >= 50 && pl.Session.Ping.AveragePing() < 100) pingColor = "♀";
            if (pl.Session.Ping.AveragePing() > 100 && pl.Session.Ping.AveragePing() < 200) pingColor = "♂";
            if (pl.Session.Ping.AveragePing() >= 200) pingColor = "◙";
